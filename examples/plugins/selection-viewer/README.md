...
上部の検索でタスク名を絞り込めます。検索中も入力済みの指示とチェックは保持されます。
チェックを付けると「まとめて実行」が開き、選択件数と対象のタスク名が表示されます。
検索で隠れているタスクも選択に含まれるため、送信前に対象を確認できます。「選択を解除」でまとめて外せます。
注意点は下部の折りたたみ欄で確認できます。狭いタブでは実行ボタンを入力欄の下へ配置します。

見た目は `panel.css` で編集します。Orca の配色トークンと同梱の Geist フォントを利用し、ビルド時に CSS・フォント・スクリプトを HTML にまとめます。外部通信は不要です。
