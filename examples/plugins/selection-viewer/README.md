...
5. 各タスクの指示欄に入力し、**このタスクを実行**を押すと、その1件とその指示だけを送信します。複数件まとめる場合はチェックして、下の一括実行用の指示欄と **実行** を使います。下部の履歴から標準の実行画面へ移動できます。「Execution ended」は実行終了を表し、業務の成功を保証する表示ではありません。
...
チェックを付けなくても、タスク内の「このタスクを実行」でその1件だけを送れます。
タスク別の指示と一括実行の指示は混ざりません。指示が空の場合はタスクのデータだけを送ります。
それぞれの受付状態を指示欄の下に表示します。受付が不明な場合は「受付状況を確認」で照会し、再送はしません。
他のタスクは独立して送信できます。送信中・受付不明の項目がある間はデータ再読込を無効にします。
