...
export async function createWebRuntimeSessionBrowserTab(
  ...
  try {
    ...
    // Why the main process owns this cache: renderer status can lag a runtime replacement. The
    // preparation path reuses verified status only for the current ready transport and probes
    // again after reconnect/restart, so an old host cannot be mistaken for a capable one.
