...
} from '../right-sidebar/plugin-panel-bridge-host'
import { createPanelWatchdog } from '../right-sidebar/plugin-panel-watchdog'
import {
  buildPanelDesignTokenCss,
  currentPanelColorScheme
} from '../right-sidebar/plugin-panel-design-token-css'
import { usePluginPanelThemeRevision } from '../right-sidebar/use-plugin-panel-theme-revision'
...
function PluginPanel({ tabKey, workspaceId, onSession }: PluginPanelProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const load = (): void => {
      ...
      pluginsApi
        .readPanelEntry({ pluginKey, panelId, ...(workspaceId ? { workspaceId } : {}) })
        .then((entry) => {
  ...
  }, [panelId, pluginKey, setPanelHealth, tabKey, workspaceId])
